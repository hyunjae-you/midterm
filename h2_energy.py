...
min_idx = np.argmin(binding_energies)
print(f"최소 결합 에너지: {binding_energies[min_idx]:.6f} Ha (거리: {distances[min_idx]:.2f} Å)")
print(f"H 원자 에너지: {h_energy}")
print(f"H₂ 결합 에너지: {binding_energies[-1]}")  # 0.74Å에서 -0.1 근처
print(f"H 원자의 전자 수: {h_atom.n_electrons}")
